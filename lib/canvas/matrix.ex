defmodule Canvas.Matrix do
  ...
  def draw(%Matrix{} = matrix, %Rectangle{pos_row: y, height: h} = rectangle) do
    chars = Enum.map((0..matrix.rows - 1), fn row_idx ->
      cond do
        Enum.member?(y..(y + h - 1), row_idx) -> redraw_row(Enum.at(matrix.chars, row_idx), row_idx, rectangle)
        true -> Enum.at(matrix.chars, row_idx)
      end
    end)
    %Matrix{matrix | chars: chars}
  end

  defp redraw_row(chars_row, row_idx, %Rectangle{outline_char: outline_char, fill_char: fill_char} = rectangle) do
